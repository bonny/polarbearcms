...
global $pb_articlesopen;
$pb_articlesopen = array();
if (isset($_COOKIE["articlesopen"])) {
	$pb_articlesopen = $_COOKIE["articlesopen"]; // root,article-4,article-16,article-23,article-24,article-27,categorySettings
	$pb_articlesopen = explode(",", $pb_articlesopen);
	for ( $i = 0; $i<sizeof($pb_articlesopen); $i++ ) {
		$pb_articlesopen[$i] = str_replace("article-", "", $pb_articlesopen[$i]);
	}
}

function pb_tree_get_children_nodes($id) {
	
	global $polarbear_db, $pb_articlesopen;
		...
		foreach ($r as $row) {
			...
			?>
				{ 
					...
					<?php
					// om $pb_articlesopen och denna artikles id finns med så öppna alla kids direkt
					if (in_array($a->getId(), $pb_articlesopen)) {
						?>, children: <?php pb_tree_get_children_nodes($a->getId());
					}
					?>
