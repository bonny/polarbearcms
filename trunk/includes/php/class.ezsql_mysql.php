...
*  Small changes by Pär Thernstöm (par@marsapril.se)
...
class ezSQLcore
{
	...
	var $debug_queries = array(); // added by Bonny/MarsApril
	var $queries = array(); // this too!
	var $queryCount = 0;
	...
	*/
	...
	function debug()
	{
		...
		// added by bonny/MarsApril
		$queryMD5 = md5($this->last_query);
		if (!isset($this->debug_queries[$queryMD5])) {
			$this->debug_queries[$queryMD5] = array("query" => $this->last_query, "count" => 0);
		}
		$this->debug_queries[$queryMD5]["count"]++;
...
class ezSQL_mysql extends ezSQLcore
{
	...
	* bonny: changed no not care about magic quotes, we deal with that earlier
	* 		 also changed mysql_escape_string to mysql_real_escape_string
	*/
	...
	* $returnAsRS added by Pär Thernström to simulate a regular mysql_query-call
	*/
	function query($query, $returnAsRS = false)
	{
	    ...
	    $arrQuery = array(
	        'sql' => $query,
	        'time' => ($this->getTime() - $startTime)*1000
	    );
	    array_push($this->queries, $arrQuery);
	    $this->queryCount++;
